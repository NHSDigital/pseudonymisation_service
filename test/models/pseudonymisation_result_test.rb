...
class PseudonymisationResultTest < ActiveSupport::TestCase
  setup do
    ...
    @demographics = Demographics.new(nhs_number: '0123456789', birth_date: '1990-01-01', postcode: 'CB22 3AD')
  end
  ...
  test 'should produce pseudoID for the requested variant' do
    ...
    result = PseudonymisationResult.new(key: @key, variant: 2, demographics: @demographics, context: 'foo')
    assert result.pseudoid.starts_with?('a630b7aa4919')
  end

  test 'should error when unknown variant requested' do
    result = PseudonymisationResult.new(key: @key, variant: 'wibble', demographics: @demographics, context: 'foo')
  ...
  test 'should produce hash results' do
    actual = PseudonymisationResult.new(key: @key, variant: 1, demographics: @demographics, context: 'foo').to_h
    expected = {
      key_name: 'Primary Key One',
      variant: 1,
      demographics: { nhs_number: '0123456789', birth_date: '1990-01-01', postcode: 'CB22 3AD' },
      context: 'foo',
